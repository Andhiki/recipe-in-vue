...
export default function useDependencies(
  ...
  function getSourceValue(dep: Dependency<any>) {
    ...
    const sourceParts = source.split('.').reverse()
    const [sourceLast, ...sourceInitial] = sourceParts
    const targetParts = (dep.targetField as string).split('.').reverse()
    const [_targetLast, ...targetInitial] = targetParts

    if (index >= 0 && sourceInitial.join(',') === targetInitial.join(',')) {
      const currentParts = fieldName.split('.').reverse()
      const [_currentLast, ...currentInitial] = currentParts
    ...
    currentFieldDependencies.value?.forEach((dep) => {
      ...
      switch (dep.type) {
        ...
        case DependencyType.SETS_OPTIONS:
          if (conditionMet && 'options' in dep)
